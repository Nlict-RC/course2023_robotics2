...
class Playground:
    ...
    def on_mousepress(self,event):
        if not event.dblclick:
            if event.button == 1:  # left button change robot start position
                self.x,self.y = event.xdata,event.ydata
            if event.button == 3:  # middle button set robot goal position
                self.planning_target = np.array([event.xdata,event.ydata])
            if event.button == 2:  # right button add obstacles
    ...
    def on_press(self,event):
        if(event.key == 'escape'):
            self.set_exit()
        if(event.key == ' '):  # space key trigger global planner
